...
void Pass_One(ifstream& source, ofstream& intermediate, ofstream& listing, int& location_counter, Table& symbol_table, Table& literal_table, ENT_Table& ent_table, Table& ext_table)
{
  Table op_table;
  char buffer[80];
  char* token;
    ...
    if (strlen(buffer) > 0 && buffer[0] != '\n')
    {
      ...
      else if(strcmp(op_code, "ENT") == 0)
      {
        if(token != NULL && token[0] != ';')
        {
          ...
          {
            if(ent_table.Is_In_Table(temp))
            {
              listing << "\nENT symbol multiply defined\n";
              cerr << "Program exited abnormally\n";
              exit(1);
            }
            else if(! Is_Valid_Label(temp))
            {
              listing << "\nInvalid Label \"" << token << '\"';
              cerr << "Program exited abnormally\n";
              exit(1);
            ...
            {
              ent_table.Put_In_Table(temp, 0);
              temp = strtok(NULL, ",");
            }
          }
        }
      }
      else if(strcmp(op_code, "EXT") == 0)
      {
        if(token != NULL && token[0] != ';')
        {
          char* temp;
          ...
          while(temp != NULL && temp[0] != ';')
          {
            if(ext_table.Is_In_Table(temp))
            {
              listing << "\nEXT symbol multiply defined\n";
              cerr << "Program exited abnormally\n";
              exit(1);
            }
            else if(! Is_Valid_Label(temp))
      ...
      if (Add_Symbol)
      {
        if (symbol_table.Is_In_Table(label))
        {
          ...
          cerr << "Program exited abnormally\n";
          exit(1);
        }else if(ext_table.Is_In_Table(label))
        {
          listing << "\nExternal symbol \"" << label << "\" illegally defined";
          cerr << "Program exited abnormally\n";
          exit(1);
        }
        else
        {
          ...
          if (ent_table.Is_In_Table(label))
          {
            ent_table.Define_This(label, value);
          }
        }
      }
    }
  ...
  if(! ent_table.All_Defined()) //make sure all ENT symbols have been defined
  {
    listing << "\nNot all entry symbols defined";
    cerr << "Program exited abnormally\n";
    exit(1);
  }
