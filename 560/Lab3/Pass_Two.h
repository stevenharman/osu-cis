int Get_S(char* str, bool& relocatable, bool& External_Symbol, Table symbol_table, Table literal_table, Table ext_table, ofstream& listing)
{
...
void Pass_Two(ifstream& source, ifstream& middle, ofstream& obj, ofstream& listing, Table symbol_table, Table literal_table, ENT_Table ent_table, Table ext_table)
{
  ...
  while(!source.eof())
  {
    ...
    if (strlen(buffer) > 0 && buffer[0] != '\n')
    {
      ...
      if (op_table.Get_Value(token) > 15)
      {
        ...
        {
          data = Get_S(token2, Is_Relocatable, External_Symbol, symbol_table, literal_table, ext_table, listing);
      ...
      else
      {
        ...
        if (temp_tok[(strlen(temp_tok)-1)] == ')')
        {
          ...
          int S = Get_S(temp_tok2, Is_Relocatable, External_Symbol, symbol_table, literal_table, ext_table, listing);
        ...
        else
        {
          ...
          listing.width(2);
          listing << hex << X;
          int S = Get_S(temp_tok, Is_Relocatable, External_Symbol, symbol_table, literal_table, ext_table, listing);
