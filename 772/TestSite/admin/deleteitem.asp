<%
	...
	Else
		...
		<%
			...
			objRS2.Open "SELECT * FROM orders_itemlist WHERE item_number = " & item_id, objConn
			If objRS2.EOF Then
			...
			Else
				...
				Do Until objRS2.EOF
					objRS.Open "SELECT * FROM orders_pending WHERE order_number = " & objRS2("order_number"), objConn
					objRS.MoveFirst
					Do Until objRS.EOF
						objRS3.Open "SELECT * FROM forum_members WHERE MEMBER_ID = " & objRS("user_id"), objConn					
						objRS3.MoveFirst
						%><li><%= objRS3("M_FIRSTNAME") %> <%= objRS3("M_LASTNAME") %> - <%= objRS("date_placed") %></li><%
						objRS3.Close
						objRS.MoveNext
					Loop
					objRS.Close
